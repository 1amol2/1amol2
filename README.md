<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=8B5E3C&height=220&section=header&text=Amol%20Chandra&fontSize=55&fontColor=ffffff&animation=fadeIn"/>

<h3>
Android Developer | Kotlin | Java | Jetpack Compose | Spring Boot
</h3>

<p>
Building scalable mobile applications with modern UI, backend systems and exploring AI/ML applications.
</p>


<p>
<img src="https://komarev.com/ghpvc/?username=1amol2&label=Profile%20Views&color=8B5E3C&style=for-the-badge"/>
</p>

</div>


---

# 👋 About Me


```yaml
name: Amol Chandra

education:
  degree: B.Tech
  institute: IIT (BHU), Varanasi

role:
  - Android Developer
  - Backend Developer
  - AI/ML Enthusiast


focus:
  - Native Android Development
  - Clean Architecture
  - REST API Development
  - Full Stack Mobile Applications
  - AI-powered Applications


android:
  - Kotlin
  - Java
  - Jetpack Compose
  - XML
  - MVVM
  - Retrofit
  - Room Database


backend:
  - Spring Boot
  - MongoDB
  - REST APIs
  - JWT Authentication
  - Docker


cloud:
  - Firebase
  - Cloudinary


ai_ml:
  - Python
  - Machine Learning Fundamentals
  - Data Analysis
  - Deep Learning Basics
  - Generative AI


currently_building:
  - BuyNSell
  - Foodie


learning:
  - Advanced Android Architecture
  - System Design
  - Machine Learning
  - AI Integration
```


---

# 🚀 Featured Projects


## 🛒 BuyNSell

A college marketplace Android application that helps students buy and sell products within their campus community.


### Tech Stack

```
Java
XML
Firebase Realtime Database
Cloudinary
Glide
Material Design
```


### Features

- 🔐 User Authentication
- 📦 Product Listing
- 🖼️ Image Upload
- 💬 Real-time Chat
- 🔍 Search & Filtering
- 👤 User Profiles


🔗 Repository:
[BuyNSell](https://github.com/1amol2)



---


## 🍔 Foodie

A full-stack food delivery application built using modern Android architecture and backend technologies.


### Tech Stack

```
Kotlin
Jetpack Compose
Retrofit
MVVM
Spring Boot
MongoDB
JWT Authentication
Docker
```


### Features

- 🍽️ Restaurant Browsing
- 🥘 Menu Management
- 🛒 Shopping Cart
- 📦 Order Management
- 🔐 Secure Authentication
- 🌐 REST API Integration


🔗 Repository:
[Foodie](https://github.com/1amol2)



---


# 🛠️ Tech Stack


<div align="center">


## 📱 Android Development


<img src="https://skillicons.dev/icons?i=kotlin,java,androidstudio,firebase"/>


<br><br>


## ⚙️ Backend Development


<img src="https://skillicons.dev/icons?i=spring,mongodb,docker"/>


<br><br>


## 🤖 AI / Machine Learning


<img src="https://skillicons.dev/icons?i=python,tensorflow,pytorch"/>


<br><br>


## 🧰 Tools


<img src="https://skillicons.dev/icons?i=git,github,postman,vscode"/>


</div>



---

# 🤖 AI / ML Journey


Currently exploring:


```
🧠 Machine Learning

- Supervised Learning
- Unsupervised Learning
- Regression
- Classification
- Model Evaluation


📊 Data Science

- Python Programming
- NumPy
- Pandas
- Data Visualization


🔥 Deep Learning

- Neural Networks
- TensorFlow Basics
- PyTorch Basics
- Computer Vision Fundamentals


🚀 AI Engineering

- AI Model Integration
- Generative AI
- LLM Fundamentals
- AI-powered Applications
```


---

# 📚 Currently Learning


```
📌 Advanced Android Architecture

📌 Jetpack Compose Best Practices

📌 Backend System Design

📌 Cloud Deployment

📌 Data Structures & Algorithms

📌 Machine Learning & AI Applications
```


---

# 🏆 Achievements


| Achievement | Details |
|---|---|
| 📱 Android Development | Built full-stack Android applications |
| ⚙️ Backend Development | Spring Boot + MongoDB REST APIs |
| 🔥 Firebase | Authentication & Realtime Database |
| 🤖 AI/ML | Exploring ML algorithms and AI applications |



---

# 📊 GitHub Analytics


<div align="center">

<img src="https://github-readme-stats-eight-theta.vercel.app/api?username=1amol2&show_icons=true&theme=tokyonight&hide_border=true"/>



<img src="https://github-readme-streak-stats.herokuapp.com/?user=1amol2&theme=tokyonight&hide_border=true"/>


</div>


---

# 📫 Connect With Me


<div align="center">


<a href="https://www.fiverr.com/amol_chandra">

<img src="https://img.shields.io/badge/Fiverr-Hire%20Me-1DBF73?style=for-the-badge&logo=fiverr&logoColor=white"/>

</a>


<a href="mailto:amolchandra010@gmail.com">

<img src="https://img.shields.io/badge/Gmail-Contact-red?style=for-the-badge&logo=gmail&logoColor=white"/>

</a>


<a href="https://github.com/1amol2">

<img src="https://img.shields.io/badge/GitHub-Profile-black?style=for-the-badge&logo=github"/>

</a>


</div>



---

<div align="center">


### "Building ideas into working products 🚀"


⭐ If you like my work, consider starring my repositories.


</div>


<img src="https://capsule-render.vercel.app/api?type=waving&color=8B5E3C&height=120&section=footer"/>
